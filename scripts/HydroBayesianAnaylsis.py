...
class HydroBayesianAnalysis(object):
    ...
    Add description
    """
    def __init__(self, parameter_names: List, parameter_ranges: np.ndarray, simulation_taus: np.ndarray, train_GP=False) -> None:
        ...
        if not self.train_GP:
            ...
            unit = lhs(n=self.num_params, samples=40 * self.num_params, criterion='maxmin')
            ...
            print(os.getcwd())
            with open('design_points/design_points_n=1.dat','w') as f_emulator_scores:
                for line in design_points:
                    for entry in line:
                        f_emulator_scores.write(f'{entry} ')
                    f_emulator_scores.write(f'\n')

            for k, name in enumerate(self.hydro_names):
                for j, tau in enumerate(simulation_taus):
                        with open(f'hydro_simulation_points/{name}_simulation_points_n=1_tau={tau}.dat', 'w') as f_emulator_scores:
            ...
            for k, name in enumerate(self.hydro_names):
                for i, design_point in enumerate(design_points):
                    with open(f'full_outputs/{name}_full_output_C={design_point}.dat', 'w') as f_emulator_scores:
