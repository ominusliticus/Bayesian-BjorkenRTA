...
class Sampler(object):
    ...
    def __init__(self, nwalkers, dim, logl, logp,
                 do_bmm=False, num_models=1, num_observations=None,
        ...
        * ``ratios``, the instantaneous inter-chain acceptance ratios (if requested).
        ...
        for i in tqdm(range(iterations), desc=desc, position=position):
            for j in [0, 1]:
                ...
                if self.do_bmm:
                    qslogl, qslogp, qsws = self._evaluate(qs)
                else:
            ...
            if self.do_bmm:
                p, ratios = self._temperature_swaps(self._betas, p, logpost, logl, ws)
            else:
            ...
            if self.do_bmm:
                if swap_ratios:
                    yield p, logpost, logl, ws, ratios
                else:
                    yield p, logpost, logl, ws
            else:
                if swap_ratios:
    ...
    def _temperature_swaps(self, betas, p, logpost, logl, ws=None):
        """
        ...
        for i in range(ntemps - 1, 0, -1):
            ...
            if self.do_bmm:
                wstemp = np.copy(ws[i, iperm[asel]])
                ws[i, iperm[asel]] = ws[i - 1, i1perm[asel]]
                ws[i - 1, i1perm[asel]] = wstemp
    ...
    def _expand_chain(self, nsave):
        ...
        if self._chain is None:
            ...
            if self.do_bmm:
                self._ws = np.zeros((self.ntemps, self.nwalkers, nsave,
                                     self.num_models, self.num_observations))
        else:
    ...
    def weights(self):
        """
        Matrix of weights kept: shape 
            (ntemps, nwalkers, nsteps, nmodels, nobservations)
        """
        return self._ws
